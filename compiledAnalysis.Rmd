...
gradMS %>%
  ...
  scale_fill_manual(values = c("Male" = "#7570B3FF", "Female" = "#E7298AFF"))+
  theme_minimal() +
...
```{r Plot 3 - WARREN LANGRIDGE, message=FALSE}

gradMS_pearnings %>%
  filter(gender != "All genders") %>% 
  ggplot(aes(x = yearsGrad, 
             y = earnings,
             colour = gender)) +
  geom_point(alpha = 0.5) +
  scale_colour_manual(values = c("#1F78B4FF","#FB9A99FF")) +
...
```{r Question 4 - specificDataAdjustments, results='hide'}
...
```{r Plot 4 - OISIN HEANEY}

p4 <- ggplot(pay_gap_later, aes(x = Years.Since.Graduation, y = Gender_Pay_Gap, colour = "#FB9A99FF")) +
