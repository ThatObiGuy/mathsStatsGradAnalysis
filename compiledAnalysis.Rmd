...
gradMS_Q1 <- gradMS_wider %>%
  filter(gender != "All genders") %>% # We're only interested in the difference, so we do not need the aggregate
  filter(yearsGrad == 1) %>% # We're only interested in newly graduated individuals (1 year since graduation)
...
gradMS_Q1_final %>%
  ...
  theme(axis.text.y = element_blank(),
        strip.text.y.left = element_text(angle = 0, hjust = 1)) # removes measures on y-axis, reducing visual noise
...
## Question 4 - Oisin
### Closer look at gap further along in career
...
something about further deviation... 
